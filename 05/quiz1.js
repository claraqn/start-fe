...
var page = 1 ;
var onceLook = 3; // 한번에 보일 개수
var totalPage = Math.ceil(todayPhoto.length / onceLook); //js 소숫점올리는 법 -> Math.ceil();

var $wrap = document.getElementById('wrap'); // $ 앞에 붙여주는 것 = DOM 형식의 변수라는 뜻
...
function getStartIndex(page){
    ...
    var startIndex = (page - 1)*onceLook;
    return startIndex;
}
...
function printImg(page){
    ...
    var str = '';
    for(var i=startIndex; i<startIndex+onceLook; i++){ // 한 페이지에 보여질,,
        if(todayPhoto[i]){
